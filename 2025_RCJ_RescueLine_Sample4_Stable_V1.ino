...
// #define seeGreen EyeValue_V[i] < 48 && EyeValue_H[i] < 165 && EyeValue_S[i] >= 20
#define seeBlack_mid EyeCorValue[6] >= EyeCor_BlackThreshold || EyeCorValue[7] >= EyeCor_BlackThreshold
...
void tail_detect()
{
  if(EyeCorValue[0] <= EyeCor_BlackThreshold || EyeCorValue[1] <= EyeCor_BlackThreshold)
  {
    YLED_ON();
  ...
  else if(EyeCorValue[13] <= EyeCor_BlackThreshold || EyeCorValue[12] <= EyeCor_BlackThreshold)
  {
    BLED_ON();
...
void PD_control() {
  ...
  leftSpeed = constrain(leftSpeed, 10, MaxMotorPWM+10);
  rightSpeed = constrain(rightSpeed, 10, MaxMotorPWM+10);
