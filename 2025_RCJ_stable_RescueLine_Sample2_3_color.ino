...
float Kp = 80;  // Proportional constant
float Kd = 40;    // Derivative constant
...
void loop() {
  ...
  // PrintEyeValue_V();
  detect_green();
  // if (EyeCorValue[0] <= EyeCor_BlackThreshold || EyeCorValue[13] <= EyeCor_BlackThreshold)
  //   WLED_ON();
  // else
    // WLED_OFF();
  tail_detect();
...
void tail_detect()
{
  if(EyeCorValue[0] <= EyeCor_BlackThreshold || EyeCorValue[1] <= EyeCor_BlackThreshold || EyeCorValue[2] <= EyeCor_BlackThreshold)
  {
    YLED_ON();
  ...
  else if(EyeCorValue[11] <= EyeCor_BlackThreshold || EyeCorValue[12] <= EyeCor_BlackThreshold || EyeCorValue[13] <= EyeCor_BlackThreshold)
  {
    BLED_ON();
    WheelR.CW_DirectMode(-BaseMotorPWM);
    WheelL.CW_DirectMode(BaseMotorPWM);
...
void PD_control() {
  ...
  WheelL.CW_DirectMode(leftSpeed);
  WheelR.CW_DirectMode(-rightSpeed);
